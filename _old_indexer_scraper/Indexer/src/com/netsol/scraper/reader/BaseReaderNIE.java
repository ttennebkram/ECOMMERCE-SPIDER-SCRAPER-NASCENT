...
public class BaseReaderNIE
{
    ...
    public void writeCSVline(CSVWriter out, ItemValuesBean itemBean, int itemNo) {
        try {
            //System.out.println(itemBean.getItemNo());
            String[] entries = {Integer.toString(itemNo), itemBean.getDescription(), itemBean.getItemMatGroup(), "EA", itemBean.getPrice(), "USD", itemBean.getItemNo(), itemBean.getItemLongText1(), itemBean.getImage(), itemBean.getUrl()};
	...
	{
    	Collection returnList = new LinkedHashSet<File>();
    	String test = "/Users/mbennett/data/customer/netsol/grainger/grainger-cache/products";
    	logger.info( "Looking for files in \"" + test + "\" ..." );
    	traverse( returnList, new File(test) );
        logger.info( "Found " + returnList.size() + " files" );
	}*/
    public void start(Configuration config,
                      ReaderConfiguration readerConfig,
                      ClearConfiguration clearConfiguration,
            	 ...
            	 NoSuchMethodException
    {
    	...
    	CSVWriter csvWriter = new CSVWriter(new FileWriter(config.supplierName + ".csv"), ',');
    	String[] entries = "#ItemNumber,NEW_ITEM-DESCRIPTION,NEW_ITEM-MATGROUP,NEW_ITEM-UNIT,NEW_ITEM-PRICE,NEW_ITEM-CURRENCY,NEW_ITEM-VENDORMAT,NEW_ITEM-LONGTEXT,NEW_ITEM-IMAGE,NEW_ITEM-URL".split(",");
        ...
        for (File file : files)
        {
            try
            {
                ...
                writeCSVline(csvWriter, itemBean, itemNo);
                itemNo++;
                
                if (itemNo % 1000 == 0) {
                    System.out.println("did " + itemNo + " records");
                }
     ...
     * @return
     */
    private void traverse( Collection<File>queue, File candidate ) {
    	if( candidate.isFile() ) {
            ...
            InstantiationException
    {
        ...
        for (int i = 0; i < tagInfo.size(); i++)
        {
            try
            {
                ...
                Pattern patt = Pattern.compile(selectedTag.regEx1);
