...
programs="airgas airgas-complete airgas-images airgas-extract-images airgasnonmed \
			...
			cdw cdw-indexer dell newpig newpig-indexer selenium"
...
# run_cdw-indexer - run the CDW indexer
function run_cdw-indexer {
	arg="$1"
	cdw_indexer_exec="$bash"
	cdw_indexer_program="$src_path/cdw-indexer/cdw_indexer.sh"
	
	make_cmd "cdw-indexer" "cdw-indexer" "$cdw_indexer_exec" "$cdw_indexer_program" "$arg"
}
...
# run_newpig - run the Newpig scraper
function run_newpig {
	newpig_exec="$java -jar"
	newpig_program="$src_path/newpig/newpig_scraper.jar"
	
	make_cmd "newpig" "newpig" "$newpig_exec" "$newpig_program" ""
}

# run_newpig-indexer - run the Newpig indexer
function run_newpig-indexer {
	arg="$1"
	newpig_indexer_exec="$bash"
	newpig_indexer_program="$src_path/newpig-indexer/newpig_indexer.sh"
	
	make_cmd "newpig-indexer" "newpig-indexer" "$newpig_indexer_exec" "$newpig_indexer_program" "$arg"
}
